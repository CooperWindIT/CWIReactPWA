...
export default function ViewMoreAlerts({ alertType, deptId, unitId }) {
    ...
    const fetchDashData = async () => {
        setDashLoading(true);
        try {
            const response = await fetchWithAuth(`PMMS/PMMSDashboard?Id=${sessionUserData?.OrgId}&Typeno=${alertType}&DepartmentId=${deptId}&UnitLocId=${unitId}`, {
                method: "GET",
                                ...
                                {Array.isArray(alertsData) && alertsData?.length > 0 ? (
                                    alertsData?.map((item, index) => (
                                        ...
                                        >
                                            ...
                                            <div className="mb-1">
                                                <i className="fa-solid fa-microchip text-secondary me-2"></i>
                                                <strong>Asset:</strong> {item.Col5} - {item.Col6}
                                            </div>
