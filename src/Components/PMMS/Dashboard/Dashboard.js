...
export default function PMMSDashboard() {
    ...
    // const handleScanSuccess = async (machineId, orgId) => {
    //     try {
    //         const payload = {
    //             MachineId: machineId,
    //             OrgId: orgId,
    ...
    //     if (offcanvasElement) {
    //         offcanvasElement.addEventListener("shown.bs.offcanvas", handleOffcanvasShown);
    //         offcanvasElement.addEventListener("hidden.bs.offcanvas", handleOffcanvasHidden);
    //     }

    //     return () => {
    //         if (offcanvasElement) {
    //             offcanvasElement.removeEventListener("shown.bs.offcanvas", handleOffcanvasShown);
    ...
    //             const machineId = match ? Number(match[1]) : null;
    //             if (machineId) {
    //                 handleScanSuccess(machineId);
    //             } else {
    ...
    const fetchDashData = async () => {
        setDashLoading(true);
        try {
            const response = await fetchWithAuth(`PMMS/PMMSDashboard?Id=${sessionUserData?.OrgId}&Typeno=1`, {
                method: "GET",
                headers: { "Content-Type": "application/json" },
            });
            if (response.ok) {
    ...
    // const startScannerForMachineInfo = () => {
    //     const scanner = new Html5QrcodeScanner("qr-reader", {
    //         fps: 10,
    //         qrbox: 250
    //     });
    ...
    //             // Extract OrgId and MachineId from URL
    //             const match = decodedText.match(/https?:\/\/[^\/]+\/(\d+)\/(\d+)/);
    ...
    //                 // 👇 Navigate directly after successful scan
    //                 navigate(`/ticket-info/${orgId}/${machineId}`);
    //             } else {
    //                 Swal.fire("Invalid", "Invalid QR Code format", "error");
    //             }
    //         },
        ...
        if (mcnInfoScannerStarted) {
            ...
            scanner.render(
                (decodedText) => {
                    ...
                    if (orgId && machineId) {
                        navigate(`/pmms/ticket-info/${orgId}/${machineId}`);
                    } else {
    ...
    const handleMcnInfoManualSubmit = async (e) => {
        ...
        try {
            ...
            if (response.ok) {
                const data = await response.json();
                window.location.href = `/pmms/ticket-info/${data.ResultData[0].OrgId}/${data.ResultData[0].MachineId}`;
        ...
        <Base1>
                ...
                <div id="kt_app_toolbar" className="app-toolbar py-3 py-lg-6">
                    <div id="kt_app_toolbar_container" className="app-container container-xxl d-flex flex-stack">
                        ...
                        <div className="d-flex justify-content-between">
                            ...
                            <div>
                                <button
                                    className="btn btn-sm fw-bold btn-light-info border border-info"
                                    data-bs-toggle="offcanvas"
                    ...
                    <div id="kt_app_content_container" className="app-container container-xxl">
                        <div className="row gx-5 gx-xl-10">
                                ...
                                <div className="card card-flush h-xl-100">
                                    <div className="card-header rounded bgi-no-repeat bgi-size-cover bgi-position-y-top bgi-position-x-center align-items-start h-250px" style={{ backgroundImage: 'url(assets/media/svg/shapes/top-green.png)' }} data-bs-theme="light">
                                        <h3 className="card-title align-items-start flex-column text-dark pt-4">
                                            <span className="fw-bold fs-2x mb-3">Machines</span>
                                        </h3>
                                        <div className="card-toolbar pt-5">
                                            <a href="machines" className="btn btn-sm btn-icon btn-active-color-primary btn-color-primary bg-white bg-opacity-25 bg-hover-opacity-100 bg-hover-white bg-active-opacity-25 w-20px h-20px" data-kt-menu-trigger="click" data-kt-menu-placement="bottom-end" data-kt-menu-overflow="true">
                                    ...
                                    <div className="card-body mt-n20">
                                        <div className="mt-n20 position-relative">
                                            <div className="row g-3 g-lg-6">
                                                <div
                                                    className="col-6"
                                                    style={{ marginTop: '-5.6rem', pointerEvents: 'none' }}
                                                ...
                                                <div
                                                    className="col-6"
                                                    onClick={() => setMachineStatus('BREAKDOWN')}
                                                    data-bs-toggle="offcanvas"
                                                    data-bs-target="#offcanvasRightViewMachinesData"
                                                    aria-controls="offcanvasRightViewMachinesData"
                                                >
                                                    <div className="hover-card bg-gray-100 bg-opacity-70 rounded-2 px-6 py-5">
                                                        ...
                                                        <div className="m-0" title='Breakdown Machines'>
                                                            <span className="text-gray-700 fw-bolder d-block fs-2qx lh-1 ls-n1 mb-1">
                                                                {machines?.find(item => item.Label === 'BREAKDOWN')?.Count ?? 0}
                                                            </span>
                                                            <span className="text-gray-700 fw-semibold fs-6 single-line-text ">Breakdown</span>
                                                        </div>
                                                    </div>
                                                </div>
                                                <div
                                                    className="col-6"
                                                    onClick={() => setMachineStatus('READYTOOPERATE')}
                                                    data-bs-toggle="offcanvas"
                                                    data-bs-target="#offcanvasRightViewMachinesData"
                                                    aria-controls="offcanvasRightViewMachinesData"
                                                >
                                                    <div className="hover-card bg-gray-100 bg-opacity-70 rounded-2 px-6 py-5">
                                                        ...
                                                        <div className="m-0" title='Ready to Operate Machines'>
                                                            <span className="text-gray-700 fw-bolder d-block fs-2qx lh-1 ls-n1 mb-1">
                                                                {machines?.find(item => item.Label === 'READYTOOPERATE')?.Count ?? 0}
                                                            </span>
                                                            <span className="text-gray-700 fw-semibold fs-6 single-line-text1 ">Ready to Operate</span>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                            ...
                            <div className="position-relative coming-soon-wrapper col-xl-4 mb-10">
                                <div className="card card-flush h-xl-100">
                                    <div className="card-header rounded bgi-no-repeat bgi-size-cover bgi-position-y-top bgi-position-x-center align-items-start h-250px" style={{ backgroundImage: 'url(assets/media/svg/shapes/blue1.jpeg)' }} data-bs-theme="light">
                                        <h3 className="card-title align-items-start flex-column text-dark pt-4">
                            ...
                            <div className="col-xl-6 mb-10 d-none d-md-block">
                                <div className="card card-flush h-xl-100">
                                    <div className="card-header rounded bgi-no-repeat bgi-size-cover bgi-position-y-top bgi-position-x-center align-items-start h-100px">
                                        <h3 className="card-title align-items-start flex-column text-dark pt-4">
                                            <span className="fw-bold fs-1x mb-3 text-dark">Open Tickets</span>
                                        </h3>
                                        <div className="card-toolbar pt-5"
                                            data-bs-toggle="offcanvas"
                                            data-bs-target="#offcanvasRightViewOpenTicketsData"
                                            aria-controls="offcanvasRightViewOpenTicketsData"
                                        >
                                            ...
                                            <table className='table table-bordered table-hover'>
                                                <thead>
                                                    <tr className='fw-bold'>
                                                        <th>#</th>
                                                        <th>Machine</th>
                                                        <th className="single-line-text1 ">Ticket Code</th>
                                                        {/* <th>Raised By</th> */}
                                                        <th>Status</th>
                                                        <th>Date</th>
                                                    </tr>
                                                ...
                                                <tbody>
                                                    {Array.isArray(open5Tickets) && open5Tickets.map((item, index) => (
                                                        <tr key={index}>
                                                            <td>{index + 1}</td>
                                                            <td className='single-line-text1'>{item.Label}</td>
                                                            <td>{item.Col1}</td>
                                                            {/* <td>{item.Col3}</td> */}
                                                            <td>
                                                            ...
                                                            <td className='single-line-text1'>{item.Col4}</td>
                                                        </tr>
                                                    ))}
                                                </tbody>
                                            </table>
                            ...
                            <div className="col-xl-6 mb-10 d-block d-md-none">
                                <div className="card card-flush h-xl-100">
                                    ...
                                    <div className="card-body mt-n20">
                                        {Array.isArray(open5Tickets) &&
                                            open5Tickets.map((item, index) => (
                                                <div
                                                    key={index}
                                                ...
                                                >
                                                    <div className="d-flex justify-content-between align-items-center mb-2">
                                                        ...
                                                        <span
                                                            className={`badge ${getBadgeClass(item.Col5)}`}
                                                        >
                                                            {item.Col5}
                                                        </span>
                                                    </div>
                                                    ...
                                                    <div className="mb-1">
                                                        <strong>Machine:</strong> <span>{item.Label}</span>
                                                    </div>
                                                    <div className="mb-1 ">
                                                        <strong cla>Ticket Code:</strong> <span>{item.Col1}</span>
                                                    </div>
                                                    <div className="d-flex justify-content-between mb-1">
                                                        <div>
                                                            <strong>Date:</strong> <span>{item.Col4}</span>
                                                        </div>
                                                        <div>
                                                            <strong>Raised By:</strong> <span>{item.Col3}</span>
                                                        </div>
                                                    </div>
                                                </div>
                                            ))}
                            ...
                            {/* Machine Failures Desktop */}
                            <div className="col-xl-6 mb-10 d-none d-md-block">
                                <div className="card card-flush h-xl-100">
                                    <div className="card-header rounded bgi-no-repeat bgi-size-cover bgi-position-y-top bgi-position-x-center align-items-start h-100px">
                                        <h3 className="card-title align-items-start flex-column text-white pt-4">
                                            <span className="fw-bold fs-1x mb-3 text-dark">Machine Failures</span>
                                        </h3>
                                        <div className="card-toolbar pt-5"
                                            data-bs-toggle="offcanvas"
            ...
            >
                <div className="offcanvas-header">
                    <h5 id="offcanvasRightLabel">Scan Machine QR Code for Info</h5>
                    <button
                        type="button"
                        className="btn-close"
...
.spinner-border {
  ...
  border-width: 4px;
            ...
            <ViewMachinesDataByStatus statusText={machineStatus} />
            <ViewTicketStatusByStatus statusText={ticketStatus} />
            <ViewMoreOpenTickets ticketsObj={openTickets} />
            <ViewMoreMachineFailures mcnFailObj={machineFailures} />
            <ViewMoreAlerts alertType={alertType} />
        </Base1>
    );
}
