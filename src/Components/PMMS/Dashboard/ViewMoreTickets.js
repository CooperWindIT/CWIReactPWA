...
export default function ViewTicketStatusByStatus({ statusText }) {
    ...
    const fetchTicketsDataByStatus = async () => {
        ...
        if (sessionUserData.OrgId) {
            try {
                const response = await fetchWithAuth(`PMMS/GetTicketsByStatus?Status=${statusText}&OrgId=${sessionUserData.OrgId}`, {
                    method: "GET",
        ...
        if (statusText === "OverDueTickets" || statusText === "OpenTickets") {
            const fetchTicketsDataByStatus = async () => {
                ...
                if (sessionUserData.OrgId && statusText) {
                    ...
                    try {
                        const response = await fetchWithAuth(`PMMS/${endpoint}?OrgId=${sessionUserData.OrgId}`, {
                            method: "GET",
                            headers: { "Content-Type": "application/json" },
                        });
                        if (response.ok) {
        ...
        >
            ...
            <form>
                <div className="offcanvas-header d-flex justify-content-between align-items-center">
                    <h5 id="offcanvasRightLabel" className="mb-0">{statusText}</h5>
